  ...
  return (
    <div>
      <h2 id="location-heading" data-cy="LocationHeading">
        <Translate contentKey="prospectServiceApp.location.home.title">Locations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="prospectServiceApp.location.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="prospectServiceApp.location.home.createLabel">Create new Location</Translate>
          </Link>
        </div>
      </h2>
        ...
        {locationList && locationList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="prospectServiceApp.location.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {locationList.map((location, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{location.createdAt ? <TextFormat type="date" value={location.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{location.updatedAt ? <TextFormat type="date" value={location.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="prospectServiceApp.location.home.notFound">No Locations found</Translate>
            </div>
          )
        )}
      </div>
